...
class TestClass {
    ...
    private fun setMouse(x: Float, y: Float) {
...
fun checkMouseInputs() {
    for (i in fromJson(testMouseListJson)) {
...
fun test1() {
    ...
    var counter = 0
    val test = TestClass()
    val testList = fromJson(testMouseListJson)

    for (i in testList) {
        println("${i.mouseMovementX}, ${i.mouseMovementY}")
    }
